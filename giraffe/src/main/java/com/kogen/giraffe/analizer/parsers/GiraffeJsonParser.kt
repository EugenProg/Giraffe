...
internal class GiraffeJsonParser(val mediaParsers: List<ContentParser>) : ContentParser {
    override fun parse(message: Any, context: Context): AnalysisResult? {
        val trimmedStr = message.toString().trim()
        ...
        return try {
            ...
            if (trimmedStr.startsWith("{")) {
                ...
                mainFilePath = scanJsonObject(jsonObject, context) { detectedType ->
                    foundContentType = detectedType
                }

                AnalysisResult(
                    contentType = foundContentType,
                    textContent = jsonObject.toString(2),
                    filePath = mainFilePath
                )
            } else {
                val jsonArray = JSONArray(trimmedStr)
                mainFilePath = scanJsonArray(jsonArray, context) { detectedType ->
                    foundContentType = detectedType
                }

                AnalysisResult(
                    contentType = foundContentType,
                    textContent = jsonArray.toString(2),
                    filePath = mainFilePath
                )
            }
        } catch (_: Exception) {
    ...
    private fun scanJsonObject(
        ...
        while (keys.hasNext()) {
            ...
            when (val value = json.get(key)) {
                is JSONObject -> {
                    val path = scanJsonObject(value, context, onMediaFound)
                    if (firstFilePath == null) firstFilePath = path
                }

                is JSONArray -> {
                    val path = scanJsonArray(value, context, onMediaFound)
                    if (firstFilePath == null) firstFilePath = path
                }

                is String -> {
                    for (parser in mediaParsers) {
                        val mediaResult = parser.parse(value, context)
                        if (mediaResult != null && mediaResult.filePath != null) {
                            onMediaFound(mediaResult.contentType)
                            json.put(key, "file://${mediaResult.filePath}")
            ...
            when (val value = array.get(i)) {
                ...
                is String -> {
                    for (parser in mediaParsers) {
                        ...
                        if (mediaResult != null && mediaResult.filePath != null) {
                            onMediaFound(mediaResult.contentType)
                            array.put(i, "file://${mediaResult.filePath}")
                            if (firstFilePath == null) firstFilePath = mediaResult.filePath
                            break
                        }
                    }
